<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Modifier le Patient') }} — {{ $patient->name }}
        </h2>
    </x-slot>
    ...
    <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:24px;">
        <div>
            ...
            <p style="font-size:13px; color:#7f8c8d;" class="dark:text-slate-400">Mise à jour des informations du patient</p>
        </div>
        <div style="display:flex; gap:10px;">
            <a href="{{ route('patients.show', $patient->id) }}" class="px-6 py-2.5 bg-gray-200 dark:bg-slate-700 text-gray-700 dark:text-gray-200 rounded-2xl font-bold text-sm hover:bg-gray-300 dark:hover:bg-slate-600 transition-all">
                <i class="bi bi-arrow-left mr-1"></i> Retour à la fiche
            </a>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            ...
            <form method="POST" action="{{ route('patients.update', $patient->id) }}" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="prenom" value="{{ old('prenom', explode(' ', $patient->name)[0] ?? '') }}" 
                ...
                <div class="pt-6 border-t border-gray-50 dark:border-slate-800">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label class="text-xs font-black text-gray-400 dark:text-slate-500 uppercase tracking-widest ml-1 mb-2 block">Nouveau mot de passe</label>
                ...
                <div class="flex gap-4 pt-6">
                    ...
                    <a href="{{ route('patients.show', $patient->id) }}" class="px-8 py-4 bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-300 rounded-2xl font-black text-sm hover:bg-gray-200 transition-all">
                        Annuler
                    </a>
                </div>
            </form>
        ...
        <div class="lg:col-span-1 space-y-6">
            <div class="bg-white dark:bg-[#1e293b] rounded-3xl p-8 text-center shadow-sm border border-gray-100 dark:border-slate-700">
                <div class="w-16 h-16 rounded-2xl bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 text-2xl font-black flex items-center justify-center mx-auto mb-4">
                    {{ strtoupper(substr($patient->name, 0, 1)) }}
                </div>
                <div class="text-lg font-black text-gray-900 dark:text-white mb-1">{{ $patient->name }}</div>
                ...
                <div class="mt-8 pt-6 border-t border-gray-50 dark:border-slate-800 space-y-3">
                    <a href="{{ route('patients.historique', $patient->id) }}" class="flex items-center justify-center gap-2 w-full py-3 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 rounded-xl font-bold text-xs hover:bg-blue-100 transition-all">
