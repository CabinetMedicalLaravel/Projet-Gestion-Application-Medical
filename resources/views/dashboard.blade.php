<x-app-layout>
    <div class="py-8 bg-white dark:bg-gray-900 min-h-screen font-sans text-gray-900 dark:text-gray-100 transition-colors duration-200">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <!-- EN-TÊTE DYNAMIQUE -->
            <div class="mb-8 mt-4">
                <h1 class="text-3xl font-semibold text-gray-900 dark:text-gray-100">Bonjour, {{ Auth::user()->name }}</h1>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                ...
                <div class="bg-[#F8F7F4] dark:bg-gray-800 rounded-2xl p-6">
                    ...
                    @if(!empty($rdv))
                        <p class="text-3xl font-semibold text-gray-900 dark:text-gray-100 mb-1">
                            {{ $rdv[0]['jour'] ?? '' }} {{ $rdv[0]['mois'] ?? '' }}
                        </p>
                        <a href="{{ route('rdv.mes-rdv') }}" class="text-blue-600 font-medium text-sm hover:underline">
                            {{ $rdv[0]['medecin'] ?? '' }} — {{ $rdv[0]['heure'] ?? '' }}
                        </a>
                    @else
                ...
                <div class="bg-[#F8F7F4] dark:bg-gray-800 rounded-2xl p-6">
                    ...
                    <p class="text-3xl font-semibold text-gray-900 dark:text-gray-100 mb-1">{{ count($rdv) }}</p>
                                ...
                                <div
                                    ...
                                    <div class="flex items-center gap-4">
                                        <div
                                            ...
                                            <span
                                                class="text-xs font-semibold uppercase mt-1 leading-none">{{ $rendezVous['mois'] }}</span>
                                        </div>
                                        <div>
                                            <p class="font-semibold text-gray-900 dark:text-gray-100 text-base">{{ $rendezVous['medecin'] }}
                                            </p>
                                            <p class="text-gray-500 dark:text-gray-400 text-sm">{{ $rendezVous['specialite'] }}</p>
                                        </div>
                                    </div>
                                    <div class="text-right">
                                        <p class="font-medium text-gray-900 dark:text-gray-100 mb-1">{{ $rendezVous['heure'] }}</p>
                                        <span
                        ...
                        <ul class="space-y-5">
                            @forelse ($notifications as $notif)
                                <li class="relative pl-6 pb-5 border-b border-gray-100 dark:border-gray-700">
                                    ...
                                    <p class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ $notif['message'] }}</p>
